...
#define NO_VISIBLE "¡No se puede ver nada!"
...
struct _Game{
  ...
  char sdescription[MAX_DESCR]; /*!< Character string of the actual space */
...
char* game_get_sdescr(Game* game){
  if(!game) return NULL;

  return game->sdescription; 
}
    ...
    for(i=0;i<MAX_SPACES;i++){
      if(game_get_player_location(game)==space_get_id(game->spaces[i])){
        if(space_get_light(game->spaces[i]) == TRUE){
          strcpy(game->description, space_get_idescription(game->spaces[i]));
          return;
        }
        else{
          for(i=0; i<MAX_OBJ; i++){
          ...
          strcpy(game->description, NO_VISIBLE);  /*Se copia el mensaje de advertencia en el campo description para imprimirse posteriormente*/
          return;
        }
      }
    }
          ...
          for(i=0;i<MAX_SPACES;i++){
            if(game_get_player_location(game)==space_get_id(game->spaces[i])){
              if(space_get_light(game->spaces[i]) == TRUE){
                ...
                strcpy(game->description, NO_VISIBLE);
                return;
              }
            }
          }
...
void game_command_turnon(Game *game){
  ...
  for (i=0; i<MAX_OBJ; i++) {
    if (game_get_player_location(game->player) == game_get_object_location(game, id)){ 
      if (object_get_illuminate(game->object[i]) == TRUE) {
        object_set_turnedon(game->object[i], TRUE);
        return;
      }
    }
  }
...
void game_command_turnoff(Game *game) {
  ...
  for (i=0; i<MAX_OBJ; i++) {
    if (game_get_player_location(game->player) == game_get_object_location(game, id)) { 
      if (object_get_illuminate(game->object[i]) == TRUE) {
        object_set_turnedon(game->object[i], FALSE);
        return;
      }
    }
  }
