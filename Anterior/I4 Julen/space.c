...
struct _Space {
  ...
  char description[MAX_DESCR];  /*!< The always shown description of the space */
  char idescription[MAX_DESCR]; /*!< The descprition of the space shown after inspecting */
...
/** It gets the always shown description of a space
  */
...
/** It sets the always shown description of a space
  */
...
/** It gets the inspected description of a space
  */
const char * space_get_idesprition(Space* space){
  if(!space){
    return NULL;
  }
  return space->idescription;
}

/** It sets the inspected description of a space
  */
STATUS space_set_idescription(Space* space, char* idescription){
  if (!space || !idescription) {
    return ERROR;
  }

  if (!strcpy(space->idescription, idescription)){
    return ERROR;
  }
  return OK;
}
...
STATUS space_print(Space* space) {
  ...
  fprintf(stdout, "--> Space (Id: %ld; Name: %s; Description: %s; I-Description: %s; Illumination: %s)\n", space->id, space->name, space->description, space->idescription, space->light);
