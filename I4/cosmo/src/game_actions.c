...
Status game_actions_drop(Game *game) {
  ...
  if (object_id == NO_ID || object_id==20 || object_id==21|| object_id==22 || object_id==23) {
    return ERROR;}
...
Status game_actions_attack(Game *game) {
  ...
  character = game_get_enemie_at_location(game, player_location);
  ...
  if (num < 5) {
    ...
    if (random_target == 0) {
      ...
      game_set_player_health(game, player_health);
        ...
        for (i = 0; i < game_get_n_characters(game); i++) {
            follower = game_get_character_by_index(game, i);
            if (follower !=NULL && character_get_follow(follower) == player_get_id(game_get_player(game))) {
                random_target--;
                if (random_target == 0) {
