...
void print_combat_screen(Graphic_engine *ge, Game *game) {
  ...
  screen_area_puts(ge->map, str);
  /* Ayuda */
  screen_area_puts(ge->help, "Comandos disponibles: attack or a, Quit battle or q, Use or u");
...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game) {
  Id id_act = NO_ID, obj_loc = NO_ID,player_loc=NO_ID;
  ...
  if (game_get_state(game) == GAME_STATE_COMBAT) {
    ...
    int player_loc_prefix = player_loc / 100; 
    ...
    if (id_act != NO_ID) {
        ...
        for (i = 0; i < MAX_OBJECTS && objects[i] != NULL; i++) {
            ...
            if (obj_loc / 100 == player_loc_prefix) {
    ...
    for (i = 0; i < MAX_CHARACTERS && characters[i] != NULL; i++) {
        int char_loc = character_get_location(characters[i]);
        if (char_loc / 100 == player_loc_prefix) {
    ...
    if (player_has_object(game_get_player(game), 20) == TRUE) {
      screen_area_puts(ge->descript, "Space Ship: ");
      screen_area_puts(ge->descript, "           ___");
      screen_area_puts(ge->descript, "     |     | |");
      screen_area_puts(ge->descript, "    / \\    | |");
      screen_area_puts(ge->descript, "   |--o|===|-|");
      screen_area_puts(ge->descript, "   |---|   |d|");
      screen_area_puts(ge->descript, "  /     \\  |w|");
      screen_area_puts(ge->descript, " |       | |b|");
      screen_area_puts(ge->descript, " |       |=| |");
      screen_area_puts(ge->descript, " |       | | |");
      screen_area_puts(ge->descript, " |_______| |_|");
      screen_area_puts(ge->descript, "  |@| |@|  | |");
      screen_area_puts(ge->descript, "___________|_|_");
  } else if (player_has_object(game_get_player(game), 21)==TRUE) {
      ...
      screen_area_puts(ge->descript, "Space Ship: ");
        ...
        screen_area_puts(ge->descript, "  |      |");
        screen_area_puts(ge->descript, "  |      |");
        screen_area_puts(ge->descript, "  |______|");
        screen_area_puts(ge->descript, "   '-`'-`   .");
        screen_area_puts(ge->descript, "   / . \\'\\ . .'");
        screen_area_puts(ge->descript, "  ''( .'' ' .;'");


    } else if (player_has_object(game_get_player(game), 23)==TRUE) {
      ...
      screen_area_puts(ge->descript, "Space Ship: ");
      screen_area_puts(ge->descript, "        _________");
    screen_area_puts(ge->descript, "       (=========)");
    screen_area_puts(ge->descript, "       |=========|");
    screen_area_puts(ge->descript, "       |====_====|");
    screen_area_puts(ge->descript, "       |== / \\ ==|");
    screen_area_puts(ge->descript, "       |= / _ \\ =|");
    screen_area_puts(ge->descript, "    _  |=| ( ) |=|");
    screen_area_puts(ge->descript, "   /=\\ |=|     |=| /=\\");
    ...
    screen_area_puts(ge->descript, "   |=|/    | |    \\|=|");
    screen_area_puts(ge->descript, "   |=/     |_|     \\=|");
    screen_area_puts(ge->descript, "   |(_______________)|");
    screen_area_puts(ge->descript, "   |=| |_|__|__|_| |=|");
    screen_area_puts(ge->descript, "   |=|   ( ) ( )   |=|");
    screen_area_puts(ge->descript, "  /===\\           /===\\");
    screen_area_puts(ge->descript, " |||||||         |||||||");
    screen_area_puts(ge->descript, " -------         -------");
    ...
    sprintf(str, "     move or m (north or n, south or s, east or e, west or w, down d up u), exit or e, take or t, drop or d, attack or a, chat or c, i or inspect, k or recruit, n or abandon, u or use");
    screen_area_puts(ge->help, str);
    

    /* Paint in the feedback area */
    last_cmd = command_get_code(game_get_last_command(game));
    sprintf(str, " %s (%s): %s (P%ld)", cmd_to_str[last_cmd - NO_CMD][CMDL], cmd_to_str[last_cmd - NO_CMD][CMDS],game_get_command_status(game) == OK ? "OK" : "ERROR", player_get_id(game_get_player(game)));
    screen_area_puts(ge->feedback, str);
