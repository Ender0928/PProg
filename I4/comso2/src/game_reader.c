...
Status game_reader_load_drop_rule(Game *game, char *filename) {
    ...
    char line[WORD_SIZE] = "";
    char *toks = NULL;
    Id character_id = NO_ID, object_id = NO_ID;
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#rd:", line, 4) == 0) {
            ...
            gamerules_add_drop_rule(game_get_rules(game), character_id, object_id);
        }
    }
...
Status game_reader_open_link_rule(Game *game, char *filename) {
    ...
    char line[WORD_SIZE] = "";
    char *toks = NULL;
    Id character_id = NO_ID, link_id = NO_ID;
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#rl:", line, 4) == 0) {
            ...
            gamerules_add_open_link_on_death_rule(game_get_rules(game), character_id, link_id);
        }
    }
...
Status game_reader_load_heal_rule(Game *game, char *filename) {
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#rh:", line, 4) == 0) {
            toks = strtok(line + 4, "|");
            object_id = atol(toks);

            toks = strtok(NULL, "|");
            heal_amount = atoi(toks);

            gamerules_add_heal_on_pickup(game_get_rules(game), object_id, heal_amount);
        }
    }
...
Status game_reader_load_craft_rule(Game *game, char *filename) {
    ...
    char line[WORD_SIZE] = "";
    char *toks = NULL;
    Id object_id_1 = NO_ID, object_id_2 = NO_ID, result_object = NO_ID;
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#rc:", line, 4) == 0) {
            toks = strtok(line + 4, "|");
            object_id_1 = atol(toks);

            toks = strtok(NULL, "|");
            object_id_2 = atol(toks);

            toks = strtok(NULL, "|");
            result_object = atol(toks);

            gamerules_add_craft_rule(game_get_rules(game), object_id_1, object_id_2, result_object);
        }
    }
...
Status game_reader_add_objet_damage_rule(Game *game, char *filename) {
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#ro:", line, 4) == 0) {
            toks = strtok(line + 4, "|");
            object_id = atol(toks);

            toks = strtok(NULL, "|");
            extra_damage = atoi(toks);

            gamerules_add_object_damage_rule(game_get_rules(game), object_id, extra_damage);
        }
    }
...
Status game_reader_add_damage_enemy_rule(Game *game, char *filename) {
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#re:", line, 4) == 0) {
            ...
            gamerules_add_enemy_damage_rule(game_get_rules(game), character_id, extra_damage);
        }
    }
...
Status game_reader_add_revival_object(Game *game, char *filename) {
    ...
    while (fgets(line, WORD_SIZE, file)) {
        if (strncmp("#rr:", line, 4) == 0) {
            toks = strtok(line + 4, "|");
            object_id = atol(toks);

            gamerules_add_revival_object(game_get_rules(game), object_id);
        }
    }
