...
Id game_object_depended_at(Game* game, Id id){
    int i;
    ...
    for(i=0;i<MAX_LINK;i++){
        ...
        return object_get_dependecy(game->object[i]);
    }
    return NO_ID;
}
...
void game_command_take(Game *game){
  ...
  if(game_get_object_movable(id) == TRUE){
  dependecy = game_object_depended_at(game, id);
  ...
  for(i=0;i<MAX_SPACES;i++){
    if(game_get_player_location(game)==space_get_id(game->spaces[i])){
      if(player_inv_isFull(game->player)==FALSE && space_find_object(game->spaces[i],id)==TRUE){
          if(dependecy = NO_ID || player_find_object(game->player,dependecy)==TRUE){
  ...
  for(i=0;i<MAX_SPACES;i++){
    if(game_get_player_location(game)==space_get_id(game->spaces[i])){
      if(player_find_object(game->player,id)==TRUE){
          ...
          for(j=0;j<MAX_OBJ;j++){
              if(id == game_get_object_dependecy(game->object[j])){
                  id_aux = object_get_name(game->object[j]
                  if(player_find_object(player, id_aux))){
                      space_add_object(game->spaces[i],id_aux);
                      player_inv_del_object(game->player,id_aux);
                  }
              }
